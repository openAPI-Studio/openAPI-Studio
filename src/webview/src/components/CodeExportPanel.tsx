    ...
    if (/^\s*(#|\/\/)/.test(line)) {
      ...
      while ((m = regex.exec(line)) !== null) {
        ...
        if (m[1]) {
          ...
          const word = m[0];
          if (kw.has(word)) {
            nodes.push(<span key={m.index} style={{ color: TOKEN_COLORS.keyword }}>{word}</span>);
          } else if (/^[A-Z]/.test(word)) {
            nodes.push(<span key={m.index} style={{ color: TOKEN_COLORS.type }}>{word}</span>);
          } else {
            nodes.push(<span key={m.index} style={{ color: TOKEN_COLORS.variable }}>{word}</span>);
          }
        } else {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={copy} className="btn-ghost flex items-center gap-1 text-[11px] ml-auto opacity-50 hover:opacity-100" title="Copy to clipboard">
          <Copy size={11} /> Copy
        </button>
      </div>
