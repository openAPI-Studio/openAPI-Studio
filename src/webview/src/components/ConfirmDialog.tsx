...
export function ConfirmDialog() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }} onClick={hide}>
      <div
      ...
      >
        ...
        <div className="flex justify-end gap-2">
          <button onClick={hide} className="btn-secondary">Cancel <span className="opacity-40 ml-1 text-[10px]">Esc</span></button>
          <button onClick={() => { dialog.onConfirm(); hide(); }} className="btn-primary" style={{ background: 'var(--vsc-error)' }}>Delete</button>
        </div>
      </div>
    </div>
  );
